...
namespace CINEMA_BE.Controllers.Api
{
    public class StaffsController : ApiController
    {
        ...
        public IHttpActionResult Get(string q = "", int page = 1, int pageSize = 10)
        {
            try
            {
                ApiContext<staff> customerContext = new ApiContext<staff>(db.staffs);

                var data = customerContext.Filter(c => c.name.Contains(q)).SortBy(c => c.id, false).SelectProperties(c => new
                {
                    c.id,
                    c.name,
                    c.email,
